...
//                    this.handleDelete(e);
            ...
            handleInput() {
                ...
                if (this.stopInputEvent) {
                    this.stopInputEvent = false;
                    this.closePanel();
                    return;
                }
                const range = getPrecedingRange();
...
//                            return;
//                        }
                        this.$api.getChannelAtMembers(this.$store.state.status.currentChannelSession.ChannelId, chunk).then(data => {
                            console.log(_deepClone(data));
                            this.members = data;
                            if (this.members.length) {
                                this.openPanel(this.members, range, index);
                            } else {
                                this.closePanel();
                            }
                        });
            ...
            selectItem() {
                ...
                applyRange(r);
                applyRange(r);
                this.stopInputEvent = true;
                document.execCommand('insertText', 0, '');
                this.$emit('atSomeone', list[cur]);
            }
        },
        mounted() {
