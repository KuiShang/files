...
function initSession(sessionList) {
    ...
    if ($('#msgArea .con-list').length > 0 && appIsVisible) {
        setTimeout(function() {
...
function getOpenssionOrLoadmoreTemp(msg, hisLastMsgTime, insertPositionobj) {
    ...
    if (msg.msgtype == 6) {
        temp = tempTime +
            ...
            '<pre class="' + listText + typeClass + '"   data-msgtype="' + msg.msgtype + '"   data-senderid="' + msg.senderId + '"  data-status="' + msg.status + '"  data-sendtime="' + msg.sendtime + '">' + msgContent + resendIcon + '</pre>' +
            '</div>';
        ...
        temp = tempTime + '<div id="msg-' + msg.msgId + '" class="' + senderClass + ' clearfix">' +
            '<div imId="' + msg.imUserId + '" class="list-name" data-staff-name ="'+msg.staffName+'"  data-staff-id ="'+msg.id+'"  data-deptname ="'+msg.deptName+'" data-dutyname ="'+msg.dutyName+'" style="background:' + avatarColor + ';">' + avatarName + '</div>' +
            '<pre class="' + listText + typeClass + '" onclick="playSound(this)"   data-msgtype="' + msg.msgtype + '"   data-senderid="' + msg.senderId + '"  data-status="' + msg.status + '"  data-sendtime="' + msg.sendtime + '">' + msgContent + resendIcon + '</pre></div>';
    }
...
function openSession(session, flag) {
    ...
    if (sessionData.MsgSeqId != undefined && $(session).attr('number') && $(session).attr('number') !== '0') {
        ...
        mainObject.setMsgReaded(sessionData.GroupId, false);
    }
...
function goToMsgAreaBottom() {
    ...
    $('#msgArea-' + groupId).scrollTop($('#msgArea-' + groupId)[0].scrollHeight);
...
function loadMore_(msgArea) {
    ...
    if (SH > ST + CH) {
        $(msgArea).find('.list-new').addClass('hideList');
    }
...
function receiveMsg(msg) {
    ...
    if ($('#session-' + msg.groupid).length == 0) {
        ...
        if (msg.senderId !== myInfo.imUserid && (!appIsVisible || (appIsVisible && CHATOBJ.groupId != msg.groupid) || (appIsVisible && window.VueApp.$store.state.status.globalStatus !== 'chat'))) {
            ...
            // } else if (msg.SessionisMute === 1 && $('#session-' + msg.groupid).attr('isMute') !== '1') {
            //     $('#session-' + msg.groupid).attr('isMute', 1);
            //     $('#session-' + msg.groupid).find('.num').html('').removeClass('hideList').addClass('numsmall');
            //     $('#session-' + msg.groupid).find('.iconn-mute').removeClass('hideList');
            //     var total = parseInt($('#im-chat').attr('number')) + 1 - num_;
            //     $('#im-chat').attr('number', total);
            //     if (total > 0 && total <= 999) {
            //         $('#im-chat').find('.num').html(total).removeClass('hideList');
            //     } else if (total > 999) {
            ...
            //     }
            //     mainObject.setMacBadge(total.toString(), false);
            // } else if (msg.SessionisMute === 0 && $('#session-' + msg.groupid).attr('isMute') === '1') {
            //     $('#session-' + msg.groupid).attr('isMute', 0);
            //     $('#session-' + msg.groupid).find('.iconn-mute').addClass('hideList');
            //     if (num_ > 0 && num_ <= 999) {
            //         $('#session-' + msg.groupid).find('.num').html(num_).removeClass('hideList').removeClass('numsmall');
            //     } else if (num_ > 999) {
            //         $('#session-' + msg.groupid).find('.num').html('999+').removeClass('hideList').removeClass('numsmall');
            //     } else {
            //         $('#im-chat').find('.num').html('0').addClass('hideList');
            //     }
            //     var total = parseInt($('#im-chat').attr('number')) + num_;
        ...
        if ($('#msgArea-' + msg.groupid).length != 0) {
            ...
            if (msg.msgtype == 6) {
                ...
                temp = tempTime +
                    ...
                    '<div imId="' + msg.imUserId + '" class="list-name" data-staff-name ="'+msg.staffName+'" data-staff-id ="'+msg.id+'" data-deptname ="'+msg.deptName+'" data-dutyname ="'+msg.dutyName+'" style="background:' + avatarColor + ';">' + avatarName + '</div>' +
                    '<pre class="' + listText + typeClass + '"  data-msgtype="' + msg.msgtype + '"  data-senderid="' + msg.senderId + '"  data-status="' + msg.status + '"  data-sendtime="' + msg.sendtime + '">' + msgContent + fileStateIcon + '</pre>' +
                    '</div>';
            } else {
                temp = tempTime + '<div id="msg-' + msg.msgId + '" class="' + senderClass + ' clearfix">' +
                    ...
                    '<pre class="' + listText + typeClass + '" onclick="playSound(this)"   data-msgtype="' + msg.msgtype + '"   data-senderid="' + msg.senderId + '"  data-status="' + msg.status + '"  data-sendtime="' + msg.sendtime + '">' + msgContent + '</pre></div>';
            }
            ...
            for (var i = 0; i < msgArr.length; i++) {
                ...
                if (msgArr[i].msgType == 1) {
                    ...
                    var temp = tempTime + '<pre id="msg-' + msgId + '" status="1" class="list-right clearfix">' +
                        ...
                        '<div class="list-text" data-msgtype="1" data-senderid="' + myInfo.imUserid + '"  data-status=""  data-sendtime="' + new Date().getTime() + '"><pre>' + msgArr[i].html + '</pre>' +
                    ...
                    var temp = tempTime + '<pre id="msg-' + msgId + '" status="1" class="list-right clearfix">' +
                        '<div class="list-name" style="background:' + getNickNameColor(myInfo.myid) + ';" imId="' + myInfo.imUserid + '">我</div>' +
                        '<div class="list-text"  data-msgtype="100002"  data-senderid="' + myInfo.imUserid + '"  data-status=""  data-sendtime="' + new Date().getTime() + '"><pre>' + msgBody + '</pre>' +
                        '<em class="loading" msgId="' + msgId + '" onclick="resendMsg(this)"><img src="../images/public/loading-ms.gif"></em></div></pre>';
                    result += temp;
                    var msg = {
                        'msgid': msgId, // 类型string
                    ...
                    var msgId = Math.uuid();
                    temp = acknowledgeFnObj.getSendAcknowledgeMsgTepm(unReadNum, msgArr[i], tempTime, msgId);
                    result += temp;
                    var msg = {
                        'msgid': msgId, // 类型string
                        'msgbody': msgArr[i].msgBody, // 类型string
                    ...
                    var temp = tempTime + '<div id="msg-' + msgId + '" status="1" class="list-right clearfix">' +
                        '<div class="list-name" style="background:' + getNickNameColor(myInfo.myid) + ';" imId="' + myInfo.imUserid + '">我</div>' +
                        '<div class="list-text"  data-msgtype="3"  data-senderid="' + myInfo.imUserid + '"  data-status=""  data-sendtime="' + new Date().getTime() + '"><p><img class="pop-image"  data-picwidth="' + msgArr[i].picwidth + '"  data-picheight="' + msgArr[i].picheight + '" imgType="2" msgId="' + msgId + '" src="' + msgArr[i].picPath + '" style="max-width:150px;max-height:150px;"' +
                    ...
                    var temp = tempTime + '<div id="msg-' + msgId + '" status="1" class="list-right clearfix">' +
                        '<div class="list-name" style="background:' + getNickNameColor(myInfo.myid) + ';" imId="' + myInfo.imUserid + '">我</div>' +
                        '<div class="list-text"  data-msgtype="3"  data-senderid="' + myInfo.imUserid + '"  data-status=""  data-sendtime="' + new Date().getTime() + '"><div class="map" x="' + msgArr[i].LocaltionLongitude + '" y="' + msgArr[i].LocaltionLatitude + '" positionName="' +
                    ...
                    var msg = {
                        ...
                        LocaltionPicUrl: msgArr[i].LocaltionPicUrl.split('/').pop(),
                    ...
                    var temp = tempTime +
                        ...
                        '</div>' +
                        '<div class="icon-wrapper ' + (msgArr[i].needToDownload ? '' : 'hide') + '" data-icon-wrapper-id=' + msgId + '  data-state=' + dataState + ' data-file-path="' + msgArr[i].filePath + '"  data-file-name="' + fileAllName + '"  data-sendtime=' + nowTime + '  data-type="2"  data-senderid= ' + myInfo.imUserid + ' >' +
    ...
    nodeArr.forEach(function(item) {
        if (item.nodeType === 8) {
            ...
            var msg = {
                ...
                FileStatus: item.dataset.path ? 2 : 0,
    ...
    if (session.length == 0) {
        publicObject.getSessionInfo(groupId, function(info) {
            ...
            if (info.sessionTop == 1) {
                ...
                var temp = '<li id="session-' + groupId + '" onclick="openSession(this)"' +
...
function trigFileEv(msgId) {
    ...
    if (senderId == myInfo.imUserid) { // 自己
        if (type == PcSourceType && state !== 'notdown') { // pc
            uploadFunc();
        } else { // 移动端
            downloadFunc();
        }
                ...
                invitedArr.forEach(function(item, i) {
                    ...
                    mainObject.getOaIdByImId(uidArr[i], function(oaid) {
                        ...
                        var temp = '<li id="groupMember-' + uidArr[i] + '" imId="' + uidArr[i] + '" groupId="' + group.groupId + '"><a href="javascript:;">' +
                            ...
                            '<img class="deleteCross ' + hideListOr + '" src="../images/icon/Combined Shape@2x.png"  alt="">' +
