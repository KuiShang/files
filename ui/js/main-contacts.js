...
function initMyChannel(channel) {
    ...
    for (var i = 0; i < channel.length; i++) {
        ...
        var temp = '<li id="myChannel-' + channel[i].ChannelId + '"><a href="javascript:;">' +
            ...
            '<img class="channelAddress-icon" src="../images/icon/Group 4 Copy 7@2x.png"></div>' +
            '<p>' + codeStr(channel[i].ChannelName) + '</p></a></li>';
        $('#myChannel-content').append(temp);
        $('#myChannel-' + channel[i].ChannelId).data(channel[i]);
    }
    // 绑定点击事件
    $('#myChannel-content').on('click', 'li', function() {
...
function initMyContacts(contacts) {
	...
	// 绑定点击事件
    $('#myContacts-content').on('click', 'li', function() {
...
function initMyGroup(group) {
	...
	// 绑定点击事件
    $('#myGroup-content').on('click', 'li', function() {
...
function showChannelDetail(channel) {
    ...
    mainObject.getChannelMembers(channelInfo.ChannelId);
...
function initGroupMembers(members) {
    ...
    for (var i = 0; i < members.length; i++) {
    	...
    	var hideCross = members[i].isGourpHolder ? '' : '<img class="deleteCross hideList" src="../images/icon/Combined Shape@2x.png"  alt="">';
...
function initChannelMembers(members, channelId) {
    ...
    ret.forEach((item, idx) => {
        if (item.isLeter) {
            ...
            if (idx === 0) {
                temp += `<p class="leter first-letter">${item.isLeter.toUpperCase()}</p>`;
            } else {
