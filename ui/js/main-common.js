...
$(function() {
    new QWebChannel(qt.webChannelTransport, function(channel) {
        ...
        if (currentOsIsMac) {
            console.log($('#right-top, #im-left, #im-config, .right-con, .right-list').addClass('mac-resize'));
        }
        ...
        //         mainObject.wndEnlarge(wind.owSize);
       ...
       /* mainObject.sigGetReadAndUnReadMembers.connect(function (readedObj,unreadObj) {
            ...
            if (data.length) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    var temp = '<li class="file-item clearfix">' +
                        ...
                        '<a href="javascript:;" class="status-todo ' + statusTodo + '" statusTodo="' + statusTodo + '" gid="' + sessionData.GroupId + '" status="' + data[i].FileStatus + '" senderImId="' + data[i].senderImId + '" msgid="' + data[i].msgid + '" fpath="' + data[i].fpath + '" fileSize="' + fileSize + '" from="' + data[i].from + '" >' +
                        '</a>' +
                        ...
                        '</a>' +
                        '</div>' +
						'<a href="javascript:;" class="status-con status-text ' + statusRedFont + '" >' + fileStatus(status, senderImId, from) + '</a>' +
						'</li>';
        ...
        sendHoverImg.src = '../images/file/icon文件上传 hover@2x.png';
        downLoadHoverImg.src = '../images/file/icon文件下载   hover@2x.png';
        pauseImg.src = '../images/file/icon文件暂停 @2x.png';
        pauseHoverImg.src = '../images/file/icon文件暂停 hover@2x.png';
        moreTodoHoverImg.src = '../images/file/icon更多 hover@2x.png';
       ...
       $searchInp.on('focus', startSearch);
        ...
        function addSearchChannelsItem(data, flag) {
            ...
            data.forEach(function(item, index) {
                ...
                temp.innerHTML = '<div class="list-name" style="background:' + getNickNameColor(Math.abs(hashCode(item.ChannelId))) + ';"><img src="../images/icon/Group%204%20Copy%207@2x.png" class="channel-icon"></div>' +
        ...
        // 获取频道失败
        mainObject.sigGetChannelListFailed.connect(function() {
            console.log('sigGetChannelListFailed');
            setTimeout(() => {
        ...
        mainObject.sigGetChannelMembers.connect(function(data, channelId) {
            ...
            window.VueApp.$store.dispatch('getChannelMembers', {members: data, channelId, count: arr.length});
        });
        ...
        mainObject.SigChannelSettingFailed.connect(function(data) {
            console.log('失败设置通知', data);
            if (data === 5) {
                return;
            }
            let str = '修改失败，请稍后重试';
    ...
    $(window).resize(function() {
        ...
        var rightTitleWidth = $('.right').attr('width');
